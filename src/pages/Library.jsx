...
function Library() {
  ...
  const load = useCallback(async () => {
    ...
    setError(false)
    try {
      const [setList, folderList] = await Promise.all([getSets(), getFolders()])
  ...
  return (
    <section className="container sectionSpacing">
      <div className="introCard" role="region" aria-label="라이브러리">
        <h2>내 문장 세트</h2>
        <p>문장 세트를 선택해 학습을 시작하세요.</p>
        <p className="libraryPolicyHint">
          폴더는 문장 세트를 정리하기 위한 선택 항목입니다.
        </p>
      </div>

      <div className="introCard libraryCreateSection">
        <h3 className="librarySectionTitle">문장 세트 만들기</h3>
        <p className="libraryCreateHint">
          여러 문장을 한 번에 추가하려면 세트 만들기 화면에서 카드를 입력하세요.
        </p>
      ...
      <div className="libraryFoldersSection">
        <h3 className="librarySectionTitle">폴더</h3>
        <p className="libraryPolicyHint">폴더별로 문장 세트를 모아 볼 수 있습니다.</p>

        <div className="introCard libraryFolderCreateCard">
          <h4 className="librarySubSectionTitle">폴더 만들기</h4>
          ...
          <div className="cardGrid">
            {sortedFolders.map((folder) => (
              <article key={folder.folderId} className="learningCard libraryFolderCard">
                <Link
                  to={`/library/folders/${folder.folderId}`}
                  className="libraryFolderCardMainLink"
                >
                  <div className="libraryFolderCardHead">
                    ...
                    {folder.isDefault === 'Y' ? (
                      <span className="libraryFolderBadge">기본</span>
                    ) : null}
                  </div>
